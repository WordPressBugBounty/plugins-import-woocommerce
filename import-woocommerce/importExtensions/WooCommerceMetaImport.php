...
class WooCommerceMetaImport extends ImportHelpers{
	...
	public function woocommerce_meta_import_function ($data_array,$image_meta,$pID , $variation_id ,$import_type , $line_number, $header_array , $value_array,$mode,$hash_key) {
		global $wpdb;
		...
		foreach ($data_array as $ekey => $eval) {
			...
			case 'stock_qty' :
				...
				$parent_ids =$wpdb->get_var("SELECT post_parent FROM {$wpdb->prefix}posts WHERE ID=$pID");
				if(!empty($parent_ids)){
					update_post_meta($parent_ids,'_stock_status',$metaData['_stock_status']);
				}
			...
			case 'product_image_gallery' :
				...
				$galleryImageIds = '';
				if(isset($get_all_gallery_images) && !empty($get_all_gallery_images)){
					$indexs = 0;
					foreach($get_all_gallery_images as $gallery_image) {
						if(is_numeric($gallery_image)) {
							$galleryImageIds .= $gallery_image . ',';
						} else {
						$attachmentId = WooCommerceMetaImport::$media_instance->image_meta_table_entry($$line_number,'' ,$pID, 'product_image_gallery', $gallery_image, $hash_key, 'product', 'post','',$mode,$header_array ,$value_array,'','',$indexs);
			...
			case 'custom_attributes' :
				...
				if ($data_array[$ekey]) {					
					...
					if($product_id && !empty($vartitle) && !empty($excerpt)){
						...
						if(strpos($vartitle,'-')!==false){
							$vartitle =str_replace('-',' ',$vartitle);
						}
